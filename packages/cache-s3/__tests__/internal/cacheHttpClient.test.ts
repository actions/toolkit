...
import {ArtifactCacheEntry} from '@actions/cache/lib/internal/contracts'
import * as downloadUtils from '@actions/cache/lib/internal/downloadUtils'
import {DownloadOptions, getDownloadOptions} from '@actions/cache/lib/options'
...
test('downloadCache uses http-client for non-Azure URLs', async () => {
  ...
  const archiveLocation = {
    archiveLocation: 'http://www.actionscache.test/download'
  }
...
test('downloadCache uses storage SDK for Azure storage URLs', async () => {
  ...
  const archiveLocation: ArtifactCacheEntry = {
    archiveLocation: 'http://foo.blob.core.windows.net/bar/baz'
  }
  const archivePath = '/foo/bar'
